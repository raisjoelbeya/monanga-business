...
export const config = {
	...
	runtime: 'nodejs', // Define URL patterns to match
...
export async function middleware(request: NextRequest) {
	...
	const isProtectedApi = PROTECTED_API_PATHS.some(apiPath => pathname.startsWith(apiPath));
	
	// Gestion de la déconnexion
	const isLogoutRequest = pathname === '/login' && request.nextUrl.searchParams.get('logout') === 'true';
	...
	if (sessionId && !isPublicPath) {
		try {
			...
			if (!session?.user) {
				// Pour les API, retourner une erreur 401
				if (isProtectedApi) {
					return new NextResponse(JSON.stringify({error: 'Session expirée ou invalide'}), {
						status: 401,
						headers: {'Content-Type': 'application/json'}
					});
				}
