...
export function AuthButtons() {
  ...
  const handleAuth = async (provider: string) => {
    try {
      const response = await fetch(`/api/auth?provider=${provider}&redirect_to=${encodeURIComponent(redirectTo)}`);
  ...
  return (
    <div className="space-y-4 w-full max-w-md mx-auto">
      ...
      <button
        ...
        className="w-full flex items-center justify-center px-4 py-3 border border-blue-600 rounded-md shadow-sm text-sm font-medium text-white bg-[#1877F2] hover:bg-[#166FE5] transition-colors"
      >
